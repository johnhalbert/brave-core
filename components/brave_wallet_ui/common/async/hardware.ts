...
export async function signLedgerSolanaTransaction (
  ...
  deviceKeyring: LedgerSolanaKeyring = getLedgerSolanaHardwareKeyring()): Promise<SignHardwareTransactionOperationResult> { 
    // TODO(nvonpentz) - Is this API call necessary? I added this to match signLedgerEthereumTransaction,
    // however, I see no reason this all couldn't be done in the second API call to txService.getTransactionMessageToSign
    // In fact, it looks like that's how it's done for Filecoin.
    const nonce = await apiProxy.solanaTxManagerProxy.getNonceForSolanaHardwareTransaction(txInfo.id)
    // if (!nonce || !nonce.nonce) { // TODO what to check
    if (!nonce) {
      return { success: false, error: getLocale('braveWalletApproveTransactionError') }
    }
    const data = await apiProxy.txService.getTransactionMessageToSign(coin, txInfo.id)
