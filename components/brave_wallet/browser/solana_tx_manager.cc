...
void SolanaTxManager::OnGetLatestBlockhashHardware(
  ...
  meta->tx()->message()->set_recent_blockhash(latest_blockhash);
  meta->tx()->message()->set_last_valid_block_height(
      last_valid_block_height);  // todo keep this if it's 0?
  tx_state_manager_->AddOrUpdateTx(*meta);

  auto tx_data = meta->tx()->ToSolanaTxData();

  std::move(callback).Run(latest_blockhash);
}
...
void SolanaTxManager::GetNonceForSolanaHardwareTransaction(
    const std::string& tx_meta_id,
    GetNonceForSolanaHardwareTransactionCallback callback) {
  std::unique_ptr<SolanaTxMeta> meta =
      GetSolanaTxStateManager()->GetSolanaTx(tx_meta_id);
...
void SolanaTxManager::ProcessSolanaHardwareSignature(
  ...
  absl::optional<std::vector<uint8_t>> signatureBytes =
  ...
  // Add signature
  transaction_bytes.insert(transaction_bytes.end(), signatureBytes->begin(),
                           signatureBytes->end());
